...
class Discriminator_large(nn.Module):
  ...
  def forward(self, x, t, x_t):
    ...
    input_x = torch.cat((x, x_t), dim = 1)
    # input_x torch.Size([4, 6, 256, 256])
    h = self.start_conv(input_x)
    # start_conv torch.Size([4, 128, 256, 256])
    h = self.conv1(h,t_embed)
    # conv1 torch.Size([4, 256, 128, 128])
    h = self.conv2(h,t_embed)
    # conv2 torch.Size([4, 512, 64, 64])
    h = self.conv3(h,t_embed)
    # conv3 torch.Size([4, 512, 32, 32])
    h = self.conv4(h,t_embed)
    # conv4 torch.Size([4, 512, 16, 16])
    h = self.conv5(h,t_embed)
    # conv5 torch.Size([4, 512, 8, 8])
    out = self.conv6(h,t_embed)
    # conv6 torch.Size([4, 512, 4, 4])
    ...
    # stddev1 torch.Size([4, 1, 1, 512, 4, 4])
    stddev = torch.sqrt(stddev.var(0, unbiased=False) + 1e-8)
    # stddev2 torch.Size([1, 1, 512, 4, 4])
    stddev = stddev.mean([2, 3, 4], keepdims=True).squeeze(2)
    # stddev3 torch.Size([1, 1, 1, 1])
    stddev = stddev.repeat(group, 1, height, width)
    # stddev4 torch.Size([4, 1, 4, 4])
    out = torch.cat([out, stddev], 1)
    # out1 torch.Size([4, 513, 4, 4])
    
    out = self.final_conv(out)
    # out2 torch.Size([4, 512, 4, 4])
    out = self.act(out)
    # out3 torch.Size([4, 512, 4, 4])
    
    out = out.view(out.shape[0], out.shape[1], -1).sum(2)
    # out4 torch.Size([4, 512])
    out = self.end_linear(out)
    # out torch.Size([4, 1])
    return out
