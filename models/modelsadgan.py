...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.conv2 = nn.Conv2d(6, 3, kernel_size=1, stride=1, padding=0)
        init_scale = 0.
    ...
    # You can enable unused parameter detection by (1) passing the keyword argument `find_unused_parameters=True` to `torch.nn.parallel.DistributedDataParallel`; (2) making sure all `forward` function outputs participate in calculating loss. If you already have done the above two steps, then the distributed data parallel module wasn't able to locate the output tensors in the return value of your module's `forward` function. Please include the loss function and the structure of the return value of `forward` of your module when reporting this issue (e.g. list, dict, iterable).
    def forward(self, x, time_cond, z):
        # print("Start!!!")
        # timestep/noise_level embedding; only for continuous training
        # zemb = self.z_transform(z)
        zemb = z
        out1 = torch.cat((x, zemb), dim=1)
        x = self.conv2(out1)

        # z torch.Size([4, 3, 256, 256])
        # x torch.Size([4, 3, 256, 256])
        # out1 torch.Size([4, 6, 256, 256])
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                if h.shape[-1] in self.attn_resolutions:
                    ...
                    m_idx += 1
                hs.append(h)
                # print("h1:", h.shape)   # 6次h1,h1,h2
                # h1: torch.Size([4, 64, 256, 256])
            ...
            if i_level != self.num_resolutions - 1:
                if self.resblock_type == 'ddpm':
                    ...
                    h = modules[m_idx](hs[-1], temb)
                    # h = modules[m_idx](hs[-1], temb, zemb[-1])
                    m_idx += 1
                ...
                hs.append(h)
                # print("h2:", h.shape)
                # h2: torch.Size([4, 64, 128, 128])

        h = hs[-1]
        # h3: torch.Size([4, 256, 8, 8])
        # zemb = zemb[-1]
        h = modules[m_idx](h, temb)
        # h4: torch.Size([4, 256, 8, 8])
        # h = modules[m_idx](h, temb, zemb)
        m_idx += 1
        h = modules[m_idx](h)
        # h5: torch.Size([4, 256, 8, 8])
        m_idx += 1
        h = modules[m_idx](h, temb)
        # h6: torch.Size([4, 256, 8, 8])
        # h = modules[m_idx](h, temb, zemb)
        m_idx += 1
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
               # h = modules[m_idx](torch.cat([h, hs.pop()], dim=1), temb, zemb)
                h = modules[m_idx](torch.cat([h, hs.pop()], dim=1), temb)  # unet跳跃连接
                m_idx += 1
            #     print("h-u1:", h.shape)
